...
int main(int argc, char *argv[]) {
	...
	TriangleSoup earthSphere, sunSphere, moonSphere, venusSphere, mercurySphere, marsSphere, jupiterSphere;
    Texture earthTexture, sunTexture, moonTexture, venusTexture, mercuryTexture, marsTexture, jupiterTexture;
    Shader earthShader, sunShader, moonShader, venusShader, mercuryShader, marsShader, jupiterShader;
	...
	sunSphere.createSphere(1.0, 30);
	earthSphere.createSphere(1.0, 30);
	moonSphere.createSphere(1.0, 30);
	venusSphere.createSphere(1.0, 30);
	mercurySphere.createSphere(1.0, 30);
	marsSphere.createSphere(1.0, 30);
	jupiterSphere.createSphere(1.0, 30);

	// soupReadOBJ(&myShape, MESHFILENAME);
	earthSphere.printInfo();
	sunSphere.printInfo();
	moonSphere.printInfo();
	venusSphere.printInfo();
	mercurySphere.printInfo();
	marsSphere.printInfo();
	jupiterSphere.printInfo();

	// Create a shader program object from GLSL code in two files
	earthShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	sunShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	moonShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	venusShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	mercuryShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	marsShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	jupiterShader.createShader("vertexshader.glsl", "fragmentshader.glsl");
	...
	venusTexture.createTexture("textures/venus.tga");
	mercuryTexture.createTexture("textures/mercury.tga");
	marsTexture.createTexture("textures/mars.tga");
	jupiterTexture.createTexture("textures/jupiter.tga");
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        MVstack.push(); // Save the initial, untouched matrix
            ...
            MVstack.push(); // Save the current matrix on the stack
                ...
                // Sun
                MVstack.rotY(time/2.0f);
                MVstack.rotX(-M_PI/2); // Orient the poles along Y axis instead of Z
                MVstack.scale(0.25f); // Scale unit sphere to radius 0.5
            ...
            MVstack.rotY(0.4f*time);
            MVstack.translate(0.33f, 0.0f, 0.0f);

            MVstack.push();

                MVstack.rotY(2.0f*time); // Earth's rotation around its axis
                MVstack.rotX(-M_PI/2); // Orient the poles along Y axis instead of Z
                MVstack.scale(0.025f); // Scale unit sphere to radius 0.1
                // Update the transformation matrix in the shader
                glUniformMatrix4fv( location_MV, 1, GL_FALSE, MVstack.getCurrentMatrix() );
                // Render the geometry to draw the sun
                glBindTexture(GL_TEXTURE_2D, mercuryTexture.texID);
                earthSphere.render();
            ...
            MVstack.rotY(0.3f*time);
            MVstack.translate(0.66f, 0.0f, 0.0f);

            MVstack.push();

                MVstack.rotY(20.0f*time); // Earth's rotation around its axis
                MVstack.rotX(-M_PI/2); // Orient the poles along Y axis instead of Z
                MVstack.scale(0.05f); // Scale unit sphere to radius 0.1
                // Update the transformation matrix in the shader
                glUniformMatrix4fv( location_MV, 1, GL_FALSE, MVstack.getCurrentMatrix() );
                // Render the geometry to draw the sun
                glBindTexture(GL_TEXTURE_2D, venusTexture.texID);
                venusSphere.render();
            ...
            MVstack.translate(0.15f, 0.0f, 0.0f); // moon orbit radius

            MVstack.push(); // Save the matrix before the moon's rotation

                MVstack.rotY(1.0f*time); // moon's rotation around its axis
                MVstack.rotX(-M_PI/2); // Orient the poles along Y axis instead of Z
                MVstack.scale(0.025f); // Scale unit sphere to radius 0.1
                // Update the transformation matrix in the shader
                glUniformMatrix4fv( location_MV, 1, GL_FALSE, MVstack.getCurrentMatrix() );
            ...
            MVstack.rotY(0.1f*time);
            MVstack.translate(1.33f, 0.0f, 0.0f);
            ...
            MVstack.push();
                ...
                glUniformMatrix4fv( location_MV, 1, GL_FALSE, MVstack.getCurrentMatrix() );
                // Render the geometry to draw the sun
                glBindTexture(GL_TEXTURE_2D, marsTexture.texID);
                marsSphere.render();
            ...
            MVstack.rotY(0.05f*time);
            MVstack.translate(1.70f, 0.0f, 0.0f);

            MVstack.push();

                MVstack.rotY(0.5f*time); // Earth's rotation around its axis
                MVstack.rotX(-M_PI/2); // Orient the poles along Y axis instead of Z
                MVstack.scale(0.2f); // Scale unit sphere to radius 0.1
                // Update the transformation matrix in the shader
                glUniformMatrix4fv( location_MV, 1, GL_FALSE, MVstack.getCurrentMatrix() );
                // Render the geometry to draw the sun
                glBindTexture(GL_TEXTURE_2D, jupiterTexture.texID);
